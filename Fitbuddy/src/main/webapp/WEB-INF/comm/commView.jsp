...
<link rel=stylesheet href="<c:url value='/css/commList.css' />"
	type="text/css">
		...
		<div id="scroll">
			<div id="post_detailView">
				...
				<div class="communityProfileImage"
					style="width: 100px; height: 100px; border-radius: 50%; overflow: hidden; margin-right: 10px;">
					<c:choose>
						<c:when test="${empty community.userProfile}">
							<img src="<c:url value='/images/communityProfile.png' />"
								alt="Profile Image"
								style="width: 100%; height: 100%; object-fit: cover;">
						</c:when>
						<c:otherwise>
							<img src="<c:url value='/uploads/${community.userProfile}' />"
								alt="Profile Image"
								style="width: 100%; height: 100%; object-fit: cover;">
						</c:otherwise>
					</c:choose>
			...
			<div id="commentForm1">
				<form action="<c:url value='/community/comment' />" method="post">
					...
					<button type="submit">
						<img src="<c:url value='/images/commentClick.png' />">
					</button>
				</form>
			</div>
		</div>
